...
function completePolygonDrawing() {
    ...
    const polygonLine = new fabric.Polygon(finalPolygonPoints, { // 변수명 변경 polygonFrame -> polygonLine
        fill: 'rgba(0,0,0,0)', // 선이므로 채우기 없음
        stroke: lineColorInput.value,       // <<--- UI에서 선택한 선 색상 적용
        strokeWidth: parseInt(lineWidthInput.value, 10), // <<--- UI에서 선택한 선 굵기 적용
        id: 'polygonLine_' + Date.now() + '_' + Math.random().toString(36).substr(2, 9), // ID 접두사 변경
        customName: '선', // 기본 이름 변경
        objectCaching: false, // 선 굵기 등 변경 시 즉각 반영 위해 (선택적)
    });

    canvas.add(polygonLine); // 이 호출로 main.js의 'object:added' 이벤트가 발생하여
                            // 레이어 목록 추가, 히스토리 저장 등이 처리됨.
    canvas.setActiveObject(polygonLine); // 새로 추가된 프레임 선택

    // 그리기 모드 정상 종료를 위한 정리 작업
    polygonPoints = []; // 점 목록 초기화
    isDrawingPolygon = false; // 그리기 모드 플래그 해제
    // finishpolygonLineButton.style.display = 'none'; // toolStateManager가 버튼 display 제어
    ...
    // console.log('Polygon frame created:', polygonLine.id);
}
 ...
 */
export function initializeLineTool() {
    ...
    if (addRectFrameButton) {
        addRectFrameButton.addEventListener('click', () => {
            ...
            const frame = new fabric.Rect({ // 변수명 변경 frame -> rectLine
                left: 100, top: 100, width: 250, height: 180,
                fill: 'rgba(0,0,0,0)', // 채우기 없음
                stroke: lineColorInput.value,       // <<--- UI에서 선택한 선 색상 적용
                strokeWidth: parseInt(lineWidthInput.value, 10), // <<--- UI에서 선택한 선 굵기 적용
                id: 'rectLine_' + Date.now() + '_' + Math.random().toString(36).substr(2, 9),
                customName: '사각형 선' // 이름 변경
            });
    ...
    if (startPolygonLineButton) {
        startPolygonLineButton.addEventListener('click', () => {
            ...
            // finishpolygonLineButton.style.display = 'inline-block'; // toolStateManager가 처리
            canvas.defaultCursor = 'crosshair';
            setActiveDrawingTool('polygonLine'); // 전역 도구 상태를 'polygon'으로 (버튼 UI 업데이트 유발)
    ...
    if (finishPolygonLineButton) {
        finishPolygonLineButton.addEventListener('click', () => {
            if (isDrawingPolygon) {
                completePolygonDrawing();
            }
        });
    ...
    if (lineWidthInput && lineWidthValue) {
        lineWidthInput.addEventListener('input', (event) => {
            ...
            // 만약 선택된 선 객체가 있다면, 실시간으로 굵기 변경 (선택 사항)
            // const activeObject = canvas.getActiveObject();
            // if (activeObject && (activeObject.isType('polygon') || activeObject.isType('rect')) && activeObject.stroke) {
            //     activeObject.set('strokeWidth', parseInt(event.target.value, 10));
            //     canvas.renderAll();
            //     saveCanvasState(); // 변경 시 히스토리 저장
            // }
        });
    }
}