...
export function updateAllToolButtonsState() {
    ...
    const isPolygonDrawing = (currentActiveDrawingTool === 'polygon');
    ...
    if (startPolygonFrameButton) {
        startPolygonFrameButton.disabled = isAnyDrawingToolActive && !isPolygonDrawing; // 다른 도구 활성화 시 비활성화
        startPolygonFrameButton.style.display = isPolygonDrawing ? 'none' : 'inline-block'; // 다각형 그리기 중에는 시작 버튼 숨김
    }
    if (finishPolygonFrameButton) {
        ...
        finishPolygonFrameButton.style.display = isPolygonDrawing ? 'inline-block' : 'none'; // 다각형 그리기 중에만 완료 버튼 보임
    }
